...
const ChatComponent: React.FunctionComponent = () => {
  ...
  const { messages, sendMessage, resetChat } = useChatbot();
  ...
  return (
    <div className="flex flex-col h-[80vh] bg-white">
      <h2 className="p-4 font-semibold text-lg text-center bg-blue-100 flex text-blue-800 justify-center items-center gap-2">
        Mastery Check AI Assistant <LuBot size={25} />
      </h2>
      ...
      <div className="flex items-center p-4 bg-gray-50">
        <input
          ...
          placeholder="Start your mastery check"
          value={input}
        ...
        <button
          onClick={() => {
            if (
              window.confirm("Are you sure you want to delete current chat?")
            ) {
              resetChat();
            }
          }}
          className="p-2 bg-gray-500 text-black rounded-lg"
        >
          Reset Chat
        </button>
      </div>
